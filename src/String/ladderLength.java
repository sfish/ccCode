...
/**
 * @Title: ladderLength.java
 * @Package String
 * @Description: TODO
 * @author nutc
 * @date 2013-8-29 上午9:56:16
 * @version V1.0
 */
...
//		String[] str = { "slit", "bunk", "wars", "ping", "viva", "wynn",
...
//				"lame" };
		...
		for (int i = 0; i < str.length; i++)
			set.add(str[i]);
		System.out.println(l.ladderLength2(s, e, set));
	}
    ...
    public ArrayList<ArrayList<String>> findLadders(String start, String end, HashSet<String> dict) {
        ...
        // think about the initial condition
        if(start.length() == 0) return ret;
        if(end.length() == 0) return ret;
        ...
        int size = dict.size();
        // A * algorithm, it is still not pass the large set
        for(int i = 0; i < size; i++) {
            if(ret.size() > 0) break;
            // i is the depth of the depth first search
            findLaddersHelper(start, end, dict, list, ret, i);
        }
    ...
    public void findLaddersHelper(String current, String end, HashSet<String> dict, ArrayList<String> list, ArrayList<ArrayList<String>> ret, int depth) {
        if(depth < 0) return;
        ArrayList<String> next = getNext(current, dict);

        for(int i = 0; i < next.size(); i++) {
            // try
		    if(seen.contains(next.get(i)) || list.contains(next.get(i)) || list.contains(end)) continue;
		    list.add(next.get(i));
            seen.add(next.get(i));
			 
            if(aboutEqual(next.get(i), end) && !ret.contains(list)) {
            ...
            // redo
            findLaddersHelper(next.get(i), end, dict, list, ret, depth - 1);
            // undo
            list.remove(list.size() - 1);
            seen.remove(next.get(i));
        }
        return;
    }
    
    public ArrayList<String> getNext(String str, HashSet<String> dict){
        int misMatch = 0;
        ...
        while(itr.hasNext()) {
            ...
            if(!str.equals(next) && aboutEqual(next, str)) {
                list.add(next);
            }
        }
    ...
    public boolean aboutEqual(String str1, String str2) {
        int misMatch = 0;
        int size = str1.length() - 1;
        while(size >= 0) {
            if(str1.charAt(size) != str2.charAt(size)) {
                misMatch++;
	...
	public void getShorest(ArrayList<ArrayList<String>> ret) {
		int shortest = Integer.MAX_VALUE;
        int size = ret.size();
		for(int i = 0; i < size; i++) {
			if(ret.get(i).size() < shortest) shortest = ret.get(i).size();
		}

		for(int i = 0; i < size; i++) {
			if(ret.get(i).size() > shortest) {
                ret.remove(i);
                i--;
                size--;
			}
